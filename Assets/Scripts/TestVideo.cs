...
public class TestVideo : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        // 如果视频纹理为空或者视频还未准备好，则不进行赋值
        if (videoPlayer.texture == null || !videoPlayer.isPrepared)
        {
            return;
        }
        temp = Texture2Texture2D(videoPlayer.texture);
        temp = RunInference(temp);
        // 将VideoPlayer的视频纹理赋值给RawImage
